    ...
    public class HVRVRIKCreatorEditor : EditorWindow
    {
        ...
        private void PoseValidation()
        {
            ...
            if (GUILayout.Button("Auto Setup Prefab / HVRSettings"))
            {
                ...
                var bodyPrefab = PrefabUtility.SaveAsPrefabAssetAndConnect(avatarRoot.gameObject, $"Assets/{avatarRoot.name}.prefab", InteractionMode.UserAction);
        ...
        private void FinalSetup()
        {
            ...
            if (GUILayout.Button(_ikText))
            {
                ...
                if (_solving)
                {
                    foreach (var kvp in _cachePos) kvp.Key.localPosition = kvp.Value;
                    foreach (var kvp in _cacheRot) kvp.Key.localRotation = kvp.Value;
                ...
                else
                {
                    ...
                    var targ = vrik.solver.leftArm.target;
                    var targ2 = vrik.solver.rightArm.target;


                    if (targ) Selection.activeGameObject = targ.gameObject;
                    else if (targ2) Selection.activeGameObject = targ2.gameObject;
        ...
        private void SetupHandGrabber(HVRHandGrabber hand, out Transform ikTarget)
        {
            ...
            ikTarget = hand.transform.Find("IKTarget");
            if (!ikTarget)
            {
                var iktarg = new GameObject("IKTarget");
            ...
            so.FindProperty("HandAnimator").objectReferenceValue = avatarHand.gameObject.GetComponent<HVRHandAnimator>();
